...
class AgileBoardController < ApplicationController
  ...
  before_filter :find_optional_project, :only => [:index, :load_more]
  ...
  def update
    ...
    if @issue.save
      if @issue.status_id != params[:issue][:status_id].to_i
        status = :fail
        response = [l(:error_agile_status_transition)]
      else
        status = :ok
        response = @issue
  ...
  def load_more
    retrieve_agile_query
    @issues = @query.issues.where(:status_id => params[:column_id]).
                            limit(RedmineAgile.issues_per_column).
